...
def ensure_agent_user(agent):
    user = agent.user
    if not user:
        user = User.objects.filter(email__iexact=agent.email).first()
    if user:
        if User.objects.filter(email__iexact=agent.email).exclude(pk=user.pk).exists():
            raise serializers.ValidationError({"email": "Email is already registered."})
        linked_agent = getattr(user, "agent_profile", None)
        if linked_agent and linked_agent.id != agent.id:
            raise serializers.ValidationError({"email": "Email is already linked to another agent."})
        agent.user = user
        ...
        if agent.user is None:
            raise serializers.ValidationError({"user": "Failed to create linked user for agent."})
    ...
    if user.first_name != agent.full_name:
        user.first_name = agent.full_name
...
class ProductSerializer(serializers.ModelSerializer):
    ...
    agent_registration_code = serializers.CharField(source="agent.registration_code", read_only=True)
    agent_full_name = serializers.CharField(source="agent.full_name", read_only=True)
...
class AgentSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        # Ensure a registration code is generated server-side when not provided
        reg_code = validated_data.get("registration_code")
        if not reg_code:
            # Use the model helper to generate a unique registration code
            validated_data["registration_code"] = Agent.generate_registration_code()
        agent = super().create(validated_data)
        ensure_agent_user(agent)
        return agent
